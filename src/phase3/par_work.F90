...
SUBROUTINE PAR_WORK(FD, L,a,G, CPR,TPC, JSTRAT,NSWP, PSHBUF,PSTATS, MXTIME,INFO)
  IMPLICIT NONE

  INTEGER, INTENT(IN) :: FD(12), L,a,G, CPR,TPC, JSTRAT(2),NSWP(2)
...
#endif
#else
  IF (L .LT. 0) STOP 'PAR_WORK: L .LT. 0'
  IF (a .LT. 0) STOP 'PAR_WORK: a .LT. 0'
  IF (G .LT. 0) STOP 'PAR_WORK: G .LT. 0'
  ...
  ALLOCATE(IWORK(2*((M/2)+MOD(M,2))+6*MXNCB2),STAT=INFO)
  ...
  IFCOL = IFCOLB(I)
  K = 1 + (MAXNCOLSB - NCOLS)
  CALL BREAD_YW(FD, Y(1,K), W(1,K), L, a, IFCOL, NCOLS, SZ, INFO)
  ...
  CALL BREAD_J(FD(3), J, L, a, SZ(3), INFO)
  ...
  IFCOL = IFCOLB(I)
  K = 1 + (MAXNCOLSB - NCOLS)
  CALL BWRITE_EZS(FD(4), E(K), Z(1,K), SS(K), Y(1,K), EY(K), SY(K), W(1,K), EW(K), SW(K), L, a, G, IFCOL, NCOLS, I)
  IF (I .NE. 0) INFO = I + 1
  ! WRITE 2nd BCOL
  I = JSPAIR(2,IAM,1)
  NCOLS = NCOLSB(I)
