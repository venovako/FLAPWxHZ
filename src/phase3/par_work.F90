SUBROUTINE PAR_WORK(FD, M,N, CPR,TPC, JSTRAT,NSWP, PSHBUF,PSTATS, MXTIME,INFO)
  ...
  REAL(KIND=DWP), INTENT(OUT) :: MXTIME
  INTEGER, INTENT(OUT) :: PSTATS(8), INFO

  COMPLEX(KIND=DWP), ALLOCATABLE, TARGET :: Y(:,:),W(:,:), YU(:,:),WV(:,:), Z(:,:),ZZ(:,:)
  COMPLEX(KIND=DWP), ALLOCATABLE :: BH(:,:), BS(:,:), BZ(:,:)
  REAL(KIND=DWP), ALLOCATABLE, TARGET :: E(:), SS(:), EY(:),SY(:), EW(:),SW(:)
  INTEGER, ALLOCATABLE :: NCOLSB(:),IFCOLB(:), JSPAIR(:,:,:),JSCOMM(:,:,:)
  ...
  !DIR$ ATTRIBUTES MEMKIND:HBW, ALIGN:ALIGNB :: NCOLSB,IFCOLB, JSPAIR,JSCOMM
  ...
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: NCOLSB,IFCOLB, JSPAIR,JSCOMM
  ...
  ALLOCATE(JS(JSMLEX,3),STAT=INFO)
  ...
  JS(JSMLEX-1,1) = NSTEPS(1)
  JS(JSMLEX,1) = NPAIRS(1)
  ...
  CALL JSTRAT_SWEEP_NC(JS(1,1), NSTEPS(1), NPAIRS(1), JSPIN1, INFO)
  IF (INFO .NE. 0) STOP 'Error in JSTRAT_NEXT/UNPACK_NC(1)'

  ALLOCATE(JSPIN2(2,NPAIRS(2),NSTEPS(2)),STAT=INFO)
  IF (INFO .GT. 0) STOP 'Error allocating JSPIN2'

  CALL JSTRAT_SWEEP_NC(JS(1,2), NSTEPS(2), NPAIRS(2), JSPIN2, INFO)
  IF (INFO .NE. 0) STOP 'Error in JSTRAT_NEXT/UNPACK_NC(2)'
  ...
  CALL JSTRAT_SWEEP_WC(JS(1,3), NBSTEPS, NBPAIRS, JSPAIR, JSCOMM, INFO)
  IF (INFO .NE. 0) STOP 'Error in JSTRAT_NEXT/UNPACK_WC'
