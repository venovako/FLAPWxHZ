SUBROUTINE READCL(FN, L, a, G, CPR, TPC, JSTRAT, NSWP, INFO)
  IMPLICIT NONE
  CHARACTER(LEN=*,KIND=c_char), INTENT(OUT) :: FN
  INTEGER, INTENT(OUT) :: L, a, G, CPR, TPC, JSTRAT(2), NSWP(2), INFO

  CHARACTER(LEN=FNL) :: ARG
  INTEGER :: TMP

  IF (COMMAND_ARGUMENT_COUNT() .NE. 10) STOP 'phase3.exe FN L a G CPR TPC JSTRAT1 NSWP1 JSTRAT2 NSWP2'
  ...
  CALL GET_COMMAND_ARGUMENT(10, ARG, TMP, INFO)
  IF (INFO .NE. 0) THEN
     INFO = -10
     RETURN
  END IF
