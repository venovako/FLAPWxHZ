...
1   SZ = DIFFSZ
  ...
  SUBROUTINE BREAD_XTU(FD, X, T, U, L, G, ATOM, SZ, INFO)
    ...
    COMPLEX(KIND=DWP), INTENT(OUT), TARGET :: X(2*L,G)
    COMPLEX(KIND=DWP), INTENT(OUT), TARGET :: T(2*L,2*L)
    REAL(KIND=DWP), INTENT(OUT), TARGET :: U(L)
  ...
  SUBROUTINE BWRITE_YWJ(FD, Y, W, J, L, G, ATOM, SZ, INFO)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: FD(3), L, G, ATOM
    COMPLEX(KIND=DWP), INTENT(IN), TARGET :: Y(2*L,G)
    COMPLEX(KIND=DWP), INTENT(IN), TARGET :: W(2*L,G)
