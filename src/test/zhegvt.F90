PROGRAM ZHEGVT
  ...
  COMPLEX(KIND=DWP), ALLOCATABLE, TARGET :: D(:,:)
  INTEGER, ALLOCATABLE, TARGET :: J(:)
  COMPLEX(KIND=DWP), ALLOCATABLE :: C(:,:)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: D,C, J

  COMPLEX(KIND=DWP), ALLOCATABLE :: A(:,:),B(:,:), WORK(:)
  REAL(KIND=DWP), ALLOCATABLE :: W(:), RWORK(:)
  ...
  REAL(KIND=DWP), ALLOCATABLE, TARGET :: E(:)
#ifdef USE_X200
...
1   SZ = DIFFSZ
  ...
  SUBROUTINE BREAD_E(FD, E, N, SZ, INFO)
    ...
    REAL(KIND=DWP), INTENT(OUT), TARGET :: E(N)
    INTEGER, INTENT(OUT) :: SZ, INFO
