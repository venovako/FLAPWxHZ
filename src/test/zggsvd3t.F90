PROGRAM ZGGSVD3T
  ...
  COMPLEX(KIND=DWP), ALLOCATABLE, TARGET :: A(:,:),B(:,:)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: A,B
  COMPLEX(KIND=DWP), ALLOCATABLE :: U(:,:),V(:,:),Q(:,:),X(:,:), WORK(:)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: U,V,Q,X
  REAL(KIND=DWP), ALLOCATABLE :: ALPHA(:),BETA(:), RWORK(:)
  ...
  END IF
  LWORK = CEILING(REAL(WORK1(1))); ALLOCATE(WORK(LWORK)); WORK = Z_ZERO
