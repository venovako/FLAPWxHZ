PROGRAM ZHZL1SAT
  ...
  INTEGER :: M,MM,N,NN,NSWP,CPR,TPC, LDH,LDS,LDZ, JSTRAT,NPAIRS,NSTEPS, I,FD(9),SZ(9), NROT(2),INFO
  ...
  COMPLEX(KIND=DWP), ALLOCATABLE, TARGET :: H(:,:),S(:,:)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: H,S
  COMPLEX(KIND=DWP), ALLOCATABLE :: Z(:,:)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: Z
  REAL(KIND=DWP), ALLOCATABLE, TARGET :: E(:),EY(:),EW(:), SY(:),SW(:),SS(:)
  ...
  INFO = BLAS_PREPARE()
  I = GET_SYS_US()
  CALL ZHZL1SA(MM,NN, H,LDH, J, S,LDS, Z,LDZ, JS,JSPAIR, NSWP,CPR,TPC, E,EY,EW, SY,SW,SS, NROT,INFO)
  I = GET_SYS_US() - I
  WRITE (UOUT,'(2(I6,A),I1,A,I3,A,2(I20,A),F15.6)') &
       M,',',N,',',JSTRAT,',',INFO,',',NROT(1),',',NROT(2),',',(I*DUS2S)
  CALL JSTRAT_FREE(JS)
...
CONTAINS
  ...
  SUBROUTINE READCL(FN, M, N, JSTRAT, NSWP, TPC, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 6) STOP 'zhzl1sat.exe FN M N JSTRAT NSWP TPC'
...
1   SZ = DIFFSZ
  ...
  SUBROUTINE BREAD_YWJ(FD, Y, LDY, W, LDW, J8, M, N, SZ, INFO)
    ...
    COMPLEX(KIND=DWP), INTENT(OUT), TARGET :: Y(LDY,N), W(LDW,N)
  ...
  SUBROUTINE BWRITE_EZS(FD, E, Z, LDZ, SS, YU, LDY, EY, SY, WV, LDW, EW, SW, M, N, SZ, INFO)
    ...
    REAL(KIND=DWP), INTENT(IN), TARGET :: E(N)
    COMPLEX(KIND=DWP), INTENT(IN), TARGET :: Z(LDZ,N)
    REAL(KIND=DWP), INTENT(IN), TARGET :: SS(N)
    REAL(KIND=DWP), INTENT(IN), TARGET :: EY(N)
    REAL(KIND=DWP), INTENT(IN), TARGET :: SY(N)
    REAL(KIND=DWP), INTENT(IN), TARGET :: EW(N)
    REAL(KIND=DWP), INTENT(IN), TARGET :: SW(N)
    COMPLEX(KIND=DWP), INTENT(IN), TARGET :: YU(LDY,N)
    COMPLEX(KIND=DWP), INTENT(IN), TARGET :: WV(LDW,N)
