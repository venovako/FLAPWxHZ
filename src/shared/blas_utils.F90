...
#include "blas_zrotm_xx.F90"
  ...
  PURE SUBROUTINE BLAS_ZROTM(SIDE, N, ZX, INCX, ZY, INCY, C1, S1, S2, C2, INFO)
    ...
    IF (AIMAG(S1) .EQ. D_ZERO) THEN
...
#ifdef HAVE_IMAGINARY
       ELSE IF (DBLE(S2) .EQ. D_ZERO) THEN
          ! RI
          CALL BLAS_ZROTM_RI(SIDE, N, ZX, INCX, ZY, INCY, C1, DBLE(S1), AIMAG(S2), C2, INFO)
#endif
       ELSE
          ! RC
       ...
       END IF
#ifdef HAVE_IMAGINARY
    ELSE IF (DBLE(S1) .EQ. D_ZERO) THEN
       ! Ix
       IF (AIMAG(S2) .EQ. D_ZERO) THEN
          ! IR
          CALL BLAS_ZROTM_IR(SIDE, N, ZX, INCX, ZY, INCY, C1, AIMAG(S1), DBLE(S2), C2, INFO)
       ELSE IF (DBLE(S2) .EQ. D_ZERO) THEN
          ! II
