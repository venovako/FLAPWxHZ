...
CONTAINS
  ...
  PURE SUBROUTINE ZKSQRT2(A, B, D, J, AA, BB, CC, DD, INFO)
    ...
    ELSE IF (DENOM .LT. D_ZERO) THEN
       ...
       AA = (AA + SQMDETK2) * DCMPLX(D_ZERO, DENOM)
       BB = BB * DCMPLX(D_ZERO, DENOM)
       CC = CC * DCMPLX(D_ZERO, DENOM)
       DD = (DD + SQMDETK2) * DCMPLX(D_ZERO, DENOM)
    ELSE ! DENOM .GT. D_ZERO
       INFO = 1
       DENOM = D_ONE / SQRT(DENOM)
       AA = (AA + SQMDETK2) * DENOM
       BB = BB * DENOM
       CC = CC * DENOM
       DD = (DD + SQMDETK2) * DENOM
    END IF
