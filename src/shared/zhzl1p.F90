! L1 double complex HZ (parallel).
SUBROUTINE ZHZL1P(K, BH,NPLUS, BS,BZ, LDB, JS,JSPAIR, NSWP,TPC, NROT,INFO)
#ifndef NDEBUG
  ...
  INTEGER, INTENT(IN) :: K,NPLUS,LDB, JS(8),JSPAIR(2,JS(8),JS(7)), NSWP,TPC
  ...
  DO SWEEP = 1, NSWP
     SNROT = 0
     DO STEP = 1, NSTEPS
        !$OMP  PARALLEL DO DEFAULT(NONE) NUM_THREADS(TPC) PROC_BIND(SPREAD)              &
  ...
  IF (NROT(1) .GT. 0) THEN
     !$OMP  PARALLEL DO DEFAULT(NONE) NUM_THREADS(TPC) PROC_BIND(SPREAD) &
     !$OMP& SHARED(BH,BS,BZ,K,NPLUS) PRIVATE(I,J,L,P,DTOL,DSCL,DTMP1,DTMP2,ZTMP1,ZTMP2)
     DO J = 1, K
