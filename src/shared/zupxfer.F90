...
SUBROUTINE ZUPXFER(IAM,BSTEP, MXNC,NCP,NCQ,NCB, M,N,K, Y,YU,LDY, W,WV,LDW, Z,ZZ,LDZ, BZ,LDB,&
  ...
  INTEGER, INTENT(IN) :: IAM,BSTEP, MXNC,NCP,NCQ,NCB, M,N,K, LDY,LDW,LDZ,LDB,&
       CPR,NBSTEPS,JSCOMM(2,CPR,NBSTEPS), NPLUS
#ifndef MKL_NEST_SEQ
  INTEGER, INTENT(IN) :: TPC
#endif
  LOGICAL, INTENT(IN) :: LNOROT
  TYPE(ZSHENT), INTENT(IN) :: PSHBUF(CPR)
  DOUBLE COMPLEX, INTENT(IN) :: Y(LDY,K),W(LDW,K),Z(LDZ,K), BZ(LDB,NCB)
  DOUBLE COMPLEX, INTENT(OUT), TARGET :: YU(LDY,K),WV(LDW,K),ZZ(LDZ,K)

  INTEGER :: I, TGT

  DOUBLE COMPLEX, POINTER, CONTIGUOUS :: SRC_Y1(:,:),SRC_W1(:,:),SRC_Z1(:,:)
  DOUBLE COMPLEX, POINTER, CONTIGUOUS :: TGT_Y1(:,:),TGT_W1(:,:),TGT_Z1(:,:)
  DOUBLE COMPLEX, POINTER, CONTIGUOUS :: SRC_Y2(:,:),SRC_W2(:,:),SRC_Z2(:,:)
  DOUBLE COMPLEX, POINTER, CONTIGUOUS :: TGT_Y2(:,:),TGT_W2(:,:),TGT_Z2(:,:)
  ...
  !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(TGT) NUM_THREADS(MAX(1,MIN(2,TPC))) PROC_BIND(CLOSE)
  TGT = BLAS_SET_NUM_THREADS(MAX(1,TPC/2))
  !$OMP SECTIONS
  !$OMP SECTION
