! TGT_F = J SRC_F
#ifdef HAVE_J_SCALE
#ifndef MKL_NEST_SEQ
SUBROUTINE ZJSCL(M,N, JF,LDF, J,JNSTIX,JNLENS,JNBLKS, TPC)
#else
SUBROUTINE ZJSCL(M,N, JF,LDF, J,JNSTIX,JNLENS,JNBLKS)
#endif
#else
#ifndef MKL_NEST_SEQ
SUBROUTINE ZJSCL(N, JF,LDF, JNSTIX,JNLENS,JNBLKS, TPC)
#else
SUBROUTINE ZJSCL(N, JF,LDF, JNSTIX,JNLENS,JNBLKS)
#endif
#endif
  IMPLICIT NONE

  INTEGER, INTENT(IN) :: N, LDF
#ifdef HAVE_J_SCALE
  INTEGER, INTENT(IN) :: M, J(M)
#endif
  INTEGER, INTENT(IN) :: JNBLKS,JNSTIX(JNBLKS),JNLENS(JNBLKS)
#ifndef MKL_NEST_SEQ
  INTEGER, INTENT(IN) :: TPC
#endif
  DOUBLE COMPLEX, INTENT(INOUT) :: JF(LDF,N)

  INTEGER :: I,I1,I2, K

#ifdef HAVE_J_SCALE
#ifndef MKL_NEST_SEQ
  !$OMP PARALLEL DO DEFAULT(NONE) SHARED(M,N,JF,J,JNSTIX,JNLENS,JNBLKS) PRIVATE(I,I1,I2,K) NUM_THREADS(TPC) PROC_BIND(CLOSE)
#endif
#else
#ifndef MKL_NEST_SEQ
  !$OMP PARALLEL DO DEFAULT(NONE) SHARED(N,JF,JNSTIX,JNLENS,JNBLKS) PRIVATE(I,I1,I2,K) NUM_THREADS(TPC) PROC_BIND(CLOSE)
#endif
#endif
  DO K = 1, N
     DO I1 = 1, JNBLKS
        I2 = JNSTIX(I1) + JNLENS(I1) - 1
        DO I = JNSTIX(I1), I2
#ifdef HAVE_J_SCALE
           IF (J(I) .EQ. -1) THEN
              JF(I,K) = -JF(I,K)
           ELSE IF (J(I) .EQ. 0) THEN
              JF(I,K) = Z_ZERO
           ELSE
              JF(I,K) = J(I) * JF(I,K)
           END IF
#else
           JF(I,K) = -JF(I,K)
#endif
        END DO
     END DO
  END DO
#ifndef MKL_NEST_SEQ
  !$OMP END PARALLEL DO
#endif
END SUBROUTINE ZJSCL
