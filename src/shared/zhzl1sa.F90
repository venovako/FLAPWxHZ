SUBROUTINE ZHZL1_PSTEP(M,N, H,LDH, S,LDS, Z,LDZ, JVEC, DTOL, NPAIRS,JS,JSPAIR, STEP,PPV,VEC, SNROT)
#ifndef NDEBUG
  ...
  INTEGER, INTENT(IN) :: M,N, LDH,LDS,LDZ, JVEC(M), NPAIRS,JS(JSMLEX),JSPAIR(2,JS(JSMLEX),JS(JSMLEX-1)), STEP,PPV,VEC
  DOUBLE PRECISION, INTENT(IN) :: DTOL(1)
  DOUBLE COMPLEX, INTENT(INOUT) :: H(LDH,N),S(LDS,N),Z(LDZ,N)
  INTEGER, INTENT(INOUT) :: SNROT(2)
  ...
  !DIR$ ASSUME_ALIGNED DTOL:ALIGNB
  !DIR$ ASSUME_ALIGNED JS:ALIGNB
  !DIR$ ASSUME_ALIGNED JSPAIR:ALIGNB
  !DIR$ ASSUME_ALIGNED SNROT:ALIGNB
  ...
  END DO
#endif
END SUBROUTINE ZHZL1_PSTEP

SUBROUTINE ZHZL1_CSCALE(M,N, H,LDH, S,LDS, Z,LDZ, JVEC, EE,EY,EW,SY,SW,SS, J)
#ifndef NDEBUG
  USE, INTRINSIC :: IEEE_ARITHMETIC
  USE, INTRINSIC :: IEEE_FEATURES
#endif
  ...
  INTEGER, INTENT(IN) :: M,N, LDH,LDS,LDZ, JVEC(M), J
  DOUBLE COMPLEX, INTENT(INOUT) :: H(LDH,N),S(LDS,N),Z(LDZ,N)
  DOUBLE PRECISION, INTENT(OUT) :: EE(N),EY(N),EW(N), SY(N),SW(N),SS(N)

  DOUBLE PRECISION :: DTMP1(DSIMDL)
  DOUBLE PRECISION :: DTMP2(DSIMDL)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: DTMP1,DTMP2
  ...
  END DO
#endif
END SUBROUTINE ZHZL1_CSCALE

! L1 double complex HZ (parallel, vectorized).
SUBROUTINE ZHZL1SA(M,N, H,LDH, JVEC, S,LDS, Z,LDZ, JS,JSPAIR, NSWP,TPC,&
  ...
  IF (TPC .GT. 0) THEN
     DO SWEEP = 1, NSWP
        ...
        SNROT = 0
        DO STEP = 1, NSTEPS
           !$OMP  PARALLEL DO DEFAULT(NONE) REDUCTION(+:SNROT) PRIVATE(VEC)            &
           !$OMP& SHARED(M,N,H,LDH,S,LDS,Z,LDZ,JVEC,DTOL,NPAIRS,JS,JSPAIR,STEP,PPV,VPS)
           DO VEC = 1, VPS
              CALL ZHZL1_PSTEP(M,N, H,LDH, S,LDS, Z,LDZ, JVEC, DTOL, NPAIRS,JS,JSPAIR, STEP,PPV,VEC, SNROT)
           END DO
           !$OMP END PARALLEL DO
        END DO
  ...
  IF (NROT(1) .GT. 0) THEN
     IF (TPC .GT. 0) THEN
        !$OMP PARALLEL DEFAULT(NONE) SHARED(M,N,H,LDH,S,LDS,Z,LDZ,JVEC,TPC,EE,EY,EW,SY,SW,SS) PRIVATE(I,J)
        I = BLAS_SET_NUM_THREADS(TPC)
        !$OMP DO
        DO J = 1, N
           CALL ZHZL1_CSCALE(M,N, H,LDH, S,LDS, Z,LDZ, JVEC, EE,EY,EW,SY,SW,SS, J)
        END DO
        !$OMP END DO
